...
@section('content')
	...
	<section class="section">
        <div class="d-flex justify-content-start align-items-center flex-wrap mb-3">
            <div class="mr-2">
                <input type="text" name="parse-url" class="form-control" placeholder="Փոստի հասցեն">
            </div>
            <div class="mr-2">
                <button class="btn btn-success" onclick="getParsedPostData()">Պատճենել</button>
            </div>
            <div class="mr-2">
                <select class="form-control" name="parse-site">
			...
			<div class="flex-grow-1 mb-2 mr-0 mr-md-2">
				<div class="input-group">
		          ...
		          <button class="input-group-text bg-success text-white" onclick="copyUrl('hy')">
					...
					<input id="post-url-input-hy"  type="text" class="form-control" disabled="disabled" placeholder="Փոստի հասցեն հայերենի համար կլինի այստեղ">
				</div>
			</div>
			<div class="flex-grow-1 mb-2 ml-0 ml-md-2">
				<div class="input-group">
					<input id="post-url-input-ru"  type="text" class="form-control" disabled="disabled" placeholder="Ссилка поста для русского будет здесь">
...
@section('scripts')
	...
	<script>
        ...
        function getParsedPostData() {
            const url = $('input[name=parse-url]').val();
            const site = $('select[name=parse-site]').val();

            $.ajax({
                url: '/cabinet/posts/parse-data',
                data: {
                    url,
                    site,
                },
                ...
                complete: NProgress.done,
                success: function (res) {
                    tinymce.get('hy_content').setContent(res.content);
                    $('input[name=hy_title]').val(res.title);
                    $('input[name=image]').val(res.image);
                    $('input[name=source]').val(res.ref);
                    $('textarea[name=hy_description]').val(res.description);
                    $('#post-image-preview').attr('src', res.image);
                },
                error: function (err) {
                    showMessage(err.responseJSON.status, err.responseJSON.message);
                }
